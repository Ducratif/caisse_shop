        ...
        for _, challenge in ipairs(lootChallenges) do
            ...
            for _, item in ipairs(items) do
                if item.rarity == requiredRarity then
                    ...
                    break -- On ne valide qu'une fois par défi
                end
            end
        end
    end
...
CreateThread(function()
    ...
    for _, saison in ipairs(expired) do
        ...
        -- Logs pour les traces côté serveur
        MySQL.insert.await('INSERT INTO lootbox_saisonpass_logs (identifier, saison_id, event, details) VALUES (?, ?, ?, ?)', {
...
------------------------------------------
-- Visiter la zone apocalypse (Prison)
...
-- Nouvelle fonction qui utilise la colonne "key"
...
-- Quand un joueur achète un item en boutique, on check le défi associé, utilise ox_inventory

-- Ce handler est compatible ESX + ox_inventory (si besoin d'autre, dis-le sur mon Discord !)
...
----------------------------------------------------------
--Compatible seulement avec la base GuerreRP, sinon vérifie les imports !
